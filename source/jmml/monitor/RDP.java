...
import jmml.monitor.errorTypeConfig;
    ...
    public RDP(String jsonFile) throws FileNotFoundException, ConfigException {
        ...
        if (raw.matrixI.length != raw.vectorMark.length) {
            ...
            for (int i = 0; i < raw.vectorMark.length; ++i) {
                if (raw.vectorMark[i] < 0) {
                    throw new jmml.monitor.ConfigException("Elemento negativo en la marca", jmml.monitor.errorTypeConfig.invalidFormatArray);
                }
            }
        }
        ...
        if (this.isExtInh()) {
            if (raw.matrixH.length != raw.matrixI.length) {
                throw new jmml.monitor.ConfigException("La cantidad de plazas  en la matriz de arcos lectores e inhibidores " +
                        "no es correcta", jmml.monitor.errorTypeConfig.invalidFormatArray);
            } else if (raw.matrixH[0].length != raw.matrixI[0].length) {
                throw new jmml.monitor.ConfigException("La cantidad de transiciones  en la matriz de arcos lectores e inhibidores " +
                        "no es correcta", jmml.monitor.errorTypeConfig.invalidFormatArray);
            } else {
                /* Chequeo de longuitud de matriz constante. */
                conlconst = -1;
